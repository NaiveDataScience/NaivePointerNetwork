...
class PointerNetwork(nn.Module):
    def __init__(self, input_size, emb_size, answer_seq_len, weight_size=128, hidden_size=128, is_GRU=True):
        ...
        # self.emb = nn.Embedding(100000, 1)  # embed inputs
        if is_GRU:
    ...
    def forward(self, input):
        ...
        # Encoding
        encoder_states, hc = self.enc(new_input.float()) # encoder_state: (bs, L, H)
        ...
        for i in range(self.answer_seq_len): # range(M)
            ...
            decoder_input = torch.max(out, 1)[1].view(batch_size, -1).float()
            probs.append(out)
